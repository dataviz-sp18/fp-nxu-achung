...
joined11 <- joined %>%
  ...
  mutate(provcd_2010=replace(as.character(joined$provcd_2010),
                             joined$provcd_2010 == "Guangxi Zhuang Autonomous Region","Guangxi"))%>%
  mutate(provcd_2014=replace(as.character(joined$provcd_2014), 
                             joined$provcd_2014 =="Xinjiang Uygur Autonomous Region", "Xinjiang"))

joined12<-joined11%>%
  mutate(provcd_2014=replace(as.character(joined$provcd_2014),
                             joined$provcd_2014 =="Guangxi Zhuang Autonomous Region", "Guangxi"))
...
Difference in Mean of Income between 2014 and 2010 with China Map
=======================================================================

```{r, echo=FALSE, include=FALSE, eval=FALSE}
# I referenced the R visual. - China Map Part I
# (http://papacochon.com/2015/10/10/Codage-10-r-visualization-1-cn-map-1/) 
# and used the dataset from ggplot2-china-map Github 
# (https://github.com/xiaohk/ggplot2-china-map)

# Read the shape file of China
...
# Read in additional datafile to merge in 
merged = read.csv('../fp-nxu-achung/merged.csv')
# Clean data set and join into one dataframe
china@data$id = rownames(china@data)
china.points = fortify(china, region="id")
china.df = join(china.points, china@data, by="id")
# Filter out extra areas that are not needed
china.df = subset(china.df, AREA > 0.005)
# Make into one dataframe that contains the identifiable China region names
china.all.df<-left_join(china.df, prov_name, by = 'NAME')
# Merge into one dataframe that contains the interested variables
